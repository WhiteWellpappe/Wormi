...
class newAOO(commands.Cog, name='newAOO'):
    ...
    @commands.has_any_role('Discord King', 'Leader', "Officer")
    async def aooreaction(self, ctx, arg):
        if ctx.guild.id!=530157406349688862:
            await ctx.send("This command is only usable in WgD for now :(")
        else:
            if arg==None:
                ...
                if int(arg)>31 or int(arg)<=0:
                    ...
                    if arg=="28":
                        if datetime.datetime.now().strftime("%m")=="02":
                            arg2="1" if int(datetime.datetime.now().strftime("%Y"))%4!=0 else arg2
                    elif arg=="29":
                        if datetime.datetime.now().strftime("%m")=="02":
                            arg2="1" if int(datetime.datetime.now().strftime("%Y"))%4==0 else arg2                            
                    elif arg=="30":
                        for x in m30:
                            arg2="1" if x==datetime.datetime.now().strftime("%m") else arg2
                    elif arg=="31":
                        for x in m31:
                            arg2='1' if x==datetime.datetime.now().strftime("%m") else arg2
                    ...
                    #await self.ann.add_reaction("1️⃣")
                    #await self.ann.add_reaction("2️⃣")
                    #await self.ann.add_reaction("3️⃣")
                    #await self.ann.add_reaction("4️⃣")
                        ...
                        try:
                            ...
                            try:
                                ...
                                if self.error_count>6:
                                    await ctx.send(f"To many errors. I am stopping the loop, please consider restarting it.\n<@228476917861449738>, check this out!")
    ...
    async def endaooreaction(self, ctx):
        if ctx.guild.id==530157406349688862:
            ...
            if ctx.channel.id==651033088943587328:
                ...
                role=ctx.guild.get_role(530162542148976661)
                await ctx.channel.set_permissions(role, read_messages=True, send_messages=False, view_channel=True, read_message_history=True)
    ...
    async def resetaoo(self, ctx, mess_id):
        if ctx.guild.id==530157406349688862:
            try:
                ...
                await ctx.send(f"{ctx.author.mention} No original announcement found. AOO registration rebooting on given message")
                self.ann=await ctx.fetch_message(mess_id)
            self.loop_active=False
