...
# This is the old "Ark of Osiris" registration cog. Its outdated and no longer supported/loaded/maintained.
...
class AOO(commands.Cog, name='AOO'):
    ...
    @commands.has_any_role('Discord King', 'Leader', "Wormanager")
    ...
    async def aoo(self, ctx, arg):
        if arg is None:
            ...
            try:
                ...
                await ctx.send(
                    f"{ctx.author.mention} Please check the command again. An error occurred and I caught it so you don't wreck the channel like Woody did once.")
            arg2 = str(int(arg) + 1)
    ...
    async def joinaoo(self, ctx, arg):
        ...
        if ctx.channel.id == 651033088943587328:
                ...
                if self.aoosat_check(ctx) == 0:
                    if self.aoosat_full(ctx) == 0:
                        ...
                        if self.sun_checkvote(ctx) == 1:
                            ...
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SATURDAY 20 UTC** You changed your vote to Saturday.")
                        else:
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SATURDAY 20 UTC** You votet for the same day.")
                    else:
                        await ctx.send(
                            f"{ctx.author.mention} Sadly the roster for SATURDAY 20 UTC is already full. Please consider playing in WgDF or CIMA.")
                else:
                ...
                if self.aoosun_check(ctx) == 0:
                    if self.aoosun_full(ctx) == 0:
                        ...
                        if self.sat_checkvote == 1:
                            ...
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SUNDAY 20 UTC** You changed your vote to Sunday.")
                        else:
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SUNDAY 20 UTC** You votet for the same day.")
                    else:
                        await ctx.send(
                            f"{ctx.author.mention} Sadly the roster for SUNDAY 20 UTC is already full. Please consider playing in WgDF or CIMA.")
                else:
            ...
            # registering for both, prefering sat
                ...
                if regsat == 0 and regsun == 0:
                    if self.aoosat_full(ctx) == 0:
                        ...
                        if self.aoosun_full(ctx) == 0:
                            self.aoosun_reg(ctx)
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SATURDAY 20 UTC** and **SUNDAY 20 UTC**. You voted for Saturday.")
                        else:
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SATURDAY 20 UTC**. Sunday is already full. You voted for Saturday.")
                    else:
                        if self.aoosun_full(ctx) == 0:
                            self.aoosun_reg(ctx)
                            self.sat_deletevote(ctx)
                            self.sun_addvote(ctx)
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SUNDAY 20 UTC**. Saturday is already full. You voted for Sunday instead.")
                        else:
                            self.sat_deletevote(ctx)
                ...
                # already registered for sun
                    ...
                    if self.aoosat_full(ctx) == 0:
                        self.aoosat_reg(ctx)
                        await ctx.send(
                            f"{ctx.author.mention} has been added to the AOO roster for **SATURDAY 20 UTC**. You were already registered for Sunday. You changed your vote to Saturday.")
                    else:
                        self.sat_deletevote(ctx)
                        self.sun_addvote(ctx)
                        await ctx.send(
                            f"{ctx.author.mention} Saturday is full already. You were already registered for Sunday. Your vote didn't change.")
                # already registered for sat
                elif regsat == 1 and regsun == 0:
                    if self.aoosun_full(ctx) == 0:
                        self.aoosun_reg(ctx)
                        await ctx.send(
                            f"{ctx.author.mention} has been added to the AOO roster for **SUNDAY 20 UTC**. You were already registered for Saturday. Your vote didn't change.")
                    else:
                ...
                # already registered for both
                    ...
                    await ctx.send(
                        f" {ctx.author.mention} You are already registered for both days. Your voted for Saturday")
            # registering for both, prefering sun
                ...
                if regsat == 0 and regsun == 0:
                    if self.aoosat_full(ctx) == 0:
                        ...
                        if self.aoosun_full(ctx) == 0:
                            self.aoosun_reg(ctx)
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SATURDAY 20 UTC** and **SUNDAY 20 UTC** You voted for Sunday.")
                        else:
                            self.sat_addvote(ctx)
                            self.sun_deletevote(ctx)
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SATURDAY 20 UTC**. Sunday is already full. Your vote changed to Saturday.")
                    else:
                        if self.aoosun_full(ctx) == 0:
                            self.aoosun_reg(ctx)
                            await ctx.send(
                                f"{ctx.author.mention} has been added to the AOO roster for **SUNDAY 20 UTC**. Saturday is already full. You voted for Sunday.")
                        else:
                            self.sat_deletevote(ctx)
                            await ctx.send(f"{ctx.author.mention} Sadly both days are full. You didn't vote.")
                # already registered for sun
                    ...
                    if self.aoosat_full(ctx) == 0:
                        self.aoosat_reg(ctx)
                        await ctx.send(
                            f"{ctx.author.mention} has been added to the AOO roster for **SATURDAY 20 UTC**. You were already registered for Sunday. Your vote didn't change.")
                    else:
                        await ctx.send(
                            f"{ctx.author.mention} Saturday is full already. You were already registered for Sunday. Your vote didn't change.")
                # already registered for sat
                    ...
                    if self.aoosun_full(ctx) == 0:
                        self.aoosun_reg(ctx)
                        await ctx.send(
                            f"{ctx.author.mention} has been added to the AOO roster for **SUNDAY 20 UTC**. You were already registered for Saturday. You voted for Sunday.")
                    else:
                        await ctx.send(
                            f"{ctx.author.mention} Sunday is full already. You were already registered for Saturday. Nothing changed.")
                # already registered for both
                elif regsat == 1 and regsun == 1:
                ...
                if registered == 0:
                    await ctx.send(
                        f"{ctx.author.mention} has been added to the AOO roster for **SUNDAY 4 UTC** at **WgDF**")
                else:
